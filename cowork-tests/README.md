...
> them from prose:
>
> - **`cowork-tests/rerecord.sh` — `>= 3.5.0` (a FLOOR).** Recording bakes
...
> - **The four `version:` inputs in the `replay` job — PINNED EXACTLY at `3.5.0`.** LINT, PRIVACY,
...
> - **The `skill-static-analysis` job's standalone `npm i -g` — PINNED EXACTLY at `3.5.0`.** That step runs
...
>
> **Why not below 2.2.0 for RECORDING (the original reason; the floor is now 3.5.0 — see above).**
