...
- **`--assert-from` hard-fails on recording-shaping drift**, so the write-back is unavailable where the recording no longer corresponds to the scenario. **12 of 13 accept it; exactly one refuses** — `cap-table-acquisition`, with *"answers drifted from the recording"*. That one needs a real re-record.
