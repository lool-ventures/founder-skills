...
- **`host-inventory`: 1.18.0 flagged our own plugin's agents; 1.19.0 fixed it, and the allow entry is
  ...
  **This makes the CLI floor load-bearing** — on 1.18.0 the current allowlist reds on 240 non-findings
  ...
  `privacy-allowlist.sh` at `>=1.19.0` — though the **repo-wide floor is now `>=1.24.0`**, raised because the
...
Comparing every on-disk `assert:` key against its cassette's frozen `scenario.assert` — **re-measured 2026-08-18: 2 lanes diverge, down from 13** (the free `--assert-from --write` remedy was applied to the rest). The remaining two are `market-sizing-smoke` (`gate_answer_count_min`, `gate_answers_delivered`, `present_files_called` authored but never evaluated) and `cap-table-acquisition` (`gate_answer_count_min`; its write-back REFUSES on answer drift, so only a re-record clears it). **Re-derive rather than trusting either number** — the historical table below is kept because the MECHANISM it teaches is permanent, not because the counts are current:
