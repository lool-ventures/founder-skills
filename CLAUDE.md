...
`market-sizing-smoke` is not merely "stale". Its on-disk scenario asserts `present_files_called: true` while the frozen 1.12.0 cassette predates hostloop `present_files`, so a plain `replay` returns `✓ success` **vacuously** and only `replay --assert-from` surfaces the failure. Treat it as a live guard that is silently green, not as ordinary drift.
