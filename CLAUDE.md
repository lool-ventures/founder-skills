...
- **`host-inventory`: 1.18.0 flagged our own plugin's agents; 1.19.0 fixed it, and the allow entry is
  ...
  **This makes the CLI floor load-bearing** — on 1.18.0 the current allowlist reds on 240 non-findings
  ...
  `privacy-allowlist.sh` at `>=1.19.0` — though the **repo-wide floor is now `>=1.20.0`**, raised for an
  unrelated recording-fidelity reason (see the 1.20.0 note below). Keep both numbers: 1.19.0 is what
...
**Generalize it, because this is not about one cassette — measured, it is about most of them.** ANY assert added to a scenario YAML after its cassette was recorded is invisible to plain `replay`: CI reports on the assert set frozen at record time, not the one in the repo. Editing an `assert:` block therefore buys nothing until a re-record, and the only free way to see the on-disk block evaluated is `replay --assert-from` (which is also why "run `--assert-from` BEFORE paying for a re-record" appears in the `critique` section above).

Comparing every on-disk `assert:` key against its cassette's frozen `scenario.assert` (2026-08-15, 21 cassettes, 20 with an on-disk scenario) — **13 lanes diverge**:
...
Two rules follow. **Never record a cassette for a scenario whose asserts are not yet written** — the recording freezes the weaker set and the stronger one never runs (live right now for `deck-review-numeric-chain`, which is uncassetted and asserts none of the three cases it exists for). And **a green CI replay is evidence about the *recorded* scenario**: read the frozen `scenario.assert` array out of the cassette before concluding a guard is live. The one-liner that produces the table above is a `json.load(cassette)["scenario"]["assert"]` vs `yaml.safe_load(scenario)["assert"]` key-set diff — cheap enough to re-run after any `assert:` edit.
