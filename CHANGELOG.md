...
- **Open gaps requiring user action before merge:**
  ...
  2. **GitHub Actions secret:** configure ONE of `ANTHROPIC_API_KEY_CI` or `CLAUDE_CODE_OAUTH_TOKEN_CI` in repo settings before the `e2e-smoke` job runs successfully on internal PRs. See the "e2e auth: three paths supported" note above for trade-offs.
