...
## What v0.4.0 said but v0.4.1/v0.4.2 changes

The v0.4.0 agent body documented the full producer-script pipeline including
WebSearch calls. That's now SKILL.md's job — the main thread orchestrates the
pipeline and performs web research before dispatching you. Your job is no
longer to orchestrate or research; it's to do isolated analytical work
(Context A) or post-compose coaching (Context B) when SKILL.md dispatches you.

v0.4.2 upgrades Context B to Mitigation 2: instead of reading the full
report.md, you receive a structured `coaching_payload` in the dispatch prompt.
You edit report.md via the EXACT uuid marker (`insertion_marker`) rather than
by appending before the `---` footer. Use Grep for idempotency checks and
run_id verification.
