...
def _coverage_line(reconciliation: dict[str, Any]) -> str:
    """Markdown wrapper. The sentence itself lives in `_reconciliation_prose` so that
    `visualize.py` renders the identical text -- see that module's docstring."""
    return _reconciliation_prose.coverage_line(reconciliation, lambda s: f"**{s}**")


def _untested_claims_line(reconciliation: dict[str, Any]) -> str:
    """Markdown wrapper. See `_coverage_line`."""
    return _reconciliation_prose.untested_claims_line(reconciliation, lambda s: f"**{s}**")
