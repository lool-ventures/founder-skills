...
- **In Claude Cowork:** go straight to Bash: `find / -path '*/skills/market-sizing/scripts/market_sizing.py' 2>/dev/null | head -5`. The Glob tool searches only the workspace directory tree; the plugin is mounted outside the workspace at `.remote-plugins/`, so Glob will never find it in Cowork.
- **Outside Cowork (workspace copy):** use `Glob` for `**/skills/market-sizing/scripts/market_sizing.py` as before.
