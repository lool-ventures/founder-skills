...
**SOURCE-OVERRIDE DECLARATION (MANDATORY).** Distinct from a null/assumed field: whenever you treat a value the source actually STATES as wrong, superseded, or a formula artifact — or make any non-obvious judgment that CHANGES the numbers (e.g. "the Convertibles sheet's outstanding column is a formula error, so these SAFEs are already converted"; "ignored row N as a sum/subtotal artifact"; "reinterpreted negative-price cells") — you MUST (a) name the override explicitly in the final presentation ("treated X as Y because Z — confirm against the source") AND flag it there as an explicit assumptions/diligence caveat, and (b) include it in the cap-base confirmation gate so the founder confirms the INTERPRETATION, not just the resulting numbers. (Do NOT expect to "add a counsel item" for it — `counsel_packet` items are rule-pack-generated from matched rules; the model cannot author one. The override's home is the report narrative's assumptions/caveats + the confirm gate, both of which you DO control.) A confident reinterpretation baked silently into a `confirmed`, zero-warning base is the failure this prevents — never let a judgment call disappear into clean-looking output. (This is an instruction, not a script gate; `W_CAP_BASE_RECONSTRUCTED` already flags a hand-built base as model-reconstructed, but it does not name the specific override — that naming is on you here.)
...
- **Cap-base confirmation (S2)** — question: "Please confirm [Company]'s cap-table base — I'll use exactly these numbers for all the math:"; option labels: `Confirmed`, `Different — I'll correct it in chat`.
- **Note "Company Capitalization" denominator (S3)** — question: "What does the note's 'Company Capitalization' clause define as the conversion denominator?"; option labels: `Fully-diluted pre-financing (common + options + as-converted SAFEs/notes, before new money)`, `Issued-and-outstanding only (common + issued options, no unallocated pool)`, `I'd need to check the note text`. Do NOT append "(Recommended)" to any label — the leading `Fully-diluted` is the stable anchor; a per-run suffix breaks the cassette's answer.
- **Note maturity default (S3)** — question: "If the note reaches maturity before a qualified financing, what happens?"; option labels: `Convert at cap`, `Repay principal`, `Extend maturity`, `Counsel review / unclear`.
- **Qualified-financing threshold (S3)** — question: "What dollar amount triggers the note's automatic conversion (its 'qualified financing' threshold)?"; free-text answer (deal-specific — no fixed labels).
- **Existing-review routing (S0)** — question: "I found an existing cap-table review for [Company]. Use it, or start fresh?"; option labels: `Use existing review`, `Start fresh`.
- **Pool top-up intent (fast-assess / priced round)** — question: "Are you planning to top up your option pool as part of this round?"; option labels: `No top-up planned`, `Top up to 10% post-money`, `Top up to 15% post-money`, `Not sure yet`.
- **Company stage (S0/S2 company-context)** — question: "What stage is [Company] at?"; option labels: `Pre-seed`, `Seed`, `Series A`, `Series B+`. (Stage only — never combine with sector in the label, e.g. NOT `Seed / B2B SaaS`; if you also need sector, raise it as a SEPARATE question.)
- **Jurisdiction structure (S2 company-context)** — question: "What is [Company]'s jurisdiction structure?"; option labels: `Israeli company`, `Delaware (already flipped)`, `Mid-flip`, `Delaware with Israeli subsidiary` (→ the `israeli | delaware | mid_flip | delaware_with_israeli_sub` enum, in that order). Use these labels verbatim — do NOT reword to `Israeli company (IL only)` / `Israeli company only` etc.
- **IIA / OCS grants (Israeli company-context)** — question: "Has [Company] received any IIA (Israel Innovation Authority / OCS) grants?"; option labels: `No IIA grants`, `Yes, has IIA grants`, `Not sure`.
- **SAFE terms (Lane-2/3 when the sheet shows amounts but not cap/discount)** — question: "Do you know the valuation cap and/or discount on the SAFEs? The spreadsheet shows amounts but not the terms."; option labels: `Yes — I'll share the terms in chat`, `Uncapped MFN SAFEs`, `Don't have the terms handy`.
