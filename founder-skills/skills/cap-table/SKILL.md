...
- **Image-only PDF guard (any `.pdf` source).** Before you rely on a cap-table read from a `.pdf` by the Read tool (vision), run `python3 scripts/pdf_probe.py "<path>"`. If it returns `image_only: true`, the PDF has no text layer and dense tables are easily under-read by vision (no OCR in the sandbox) — set `inputs.json` `metadata.extraction_mode = "vision_image_pdf"` (so `cap_state.py` emits `W_VISION_EXTRACTION_LOW_CONFIDENCE` and the artifacts carry the caveat), tell the founder the cap table is LOW-CONFIDENCE / directional, and PROCEED (degraded-but-honest — do not silently present vision numbers as authoritative). Text PDFs (`image_only: false`) read normally.
