...
|---|---|---|---|---|
| **Scenario selection** | any run | "Which scenarios should I model? (select all that apply)" | `Cap-implied SAFE snapshot` (→ `safe_conversion`), `Series A priced round` (→ `priced_round`), `Convertible note conversion at financing` (→ `note_conversion`), `Israeli ↔ Delaware flip` (→ `flip`) | Offer only the scenarios that apply to this cap table. **A convertible note outstanding rules out `safe_conversion`** — that route cannot price a note, so it refuses (`E_CAP_IMPLIED_NOTES_PRESENT`); offer `priced_round`, which converts notes and SAFEs together. |
...
| **Sector** | S0/S2 company-context | "What sector best describes [Company]?" | `Use "<sector>" — as derived from the materials` (present only when a candidate was derived), `A different sector — I'll state it`, `Not sure` | RUNTIME-LABELLED, same shape as Company name — sector maps to `sector_type` via `founder_context.py:_SECTOR_ALIASES`, not a closed enum. **On the defer write an EMPTY string, never the label text** — `init` accepts it and the resulting `W_SECTOR_TYPE_UNKNOWN` is benign here (see below); a literal "Not sure" would persist into the SHARED founder-context that other skills read. |
| **Geography** | S0/S2 company-context | "Where is [Company] based?" | `Use "<geography>" — as derived from the materials` (present only when a candidate was derived), `A different location — I'll state it`, `Not sure` | RUNTIME-LABELLED, same shape as Company name — geography is open (country/region). **On the defer write an EMPTY string, never the label text** — same reason as Sector: the founder-context is shared across skills. |
