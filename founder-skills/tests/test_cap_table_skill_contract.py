...
RULES_JSON = CAP_TABLE_DIR / "data" / "cap-table-rules.json"
...
def test_error_codes_in_prose_exist_in_scripts() -> None:
    ...
    for src_file in SCRIPTS_DIR.glob("*.py"):
        ...
        # Match "E_FOO" (quoted), the "E_FOO: message" prefix convention many
        # invariants raise with, and bare E_FOO inside f-strings / error messages.
...
def test_context_b_hard_rules_forbid_edits() -> None:
    """The agent body's Context B section must forbid editing report.md
    (insertion is main-thread-only via insert_coaching.py) and forbid
    ...
    assert "edit_report_md" in section, (
        f"{AGENT_MD.name} Context B section must forbid `edit_report_md` "
        f"(insertion is script-side via insert_coaching.py)"
    )
    assert "read_full_report_md" in section, f"{AGENT_MD.name} Context B section must forbid `read_full_report_md`"
...
def test_context_b_commentary_payload_keys() -> None:
    ...
    for key in {"scenario_digest", "counsel_review_summary", "high_severity_warnings"}:
...
def test_skill_canonical_gate_phrasing_present() -> None:
    ...
    Anchored on the BLOCK'S HEADER SENTENCE, not the bare phrase 'Gate Catalog' — the
    gate-contract-plan Phase 2 sweep added several sites that now cross-reference
    specific catalog rows by name ("Gate Catalog row *Founder-only fact gates*", etc.),
    and the bare phrase's first occurrence moved earlier in the file than the actual
    table, silently pointing this window at the wrong block.

    Bounded on STRUCTURE (the last table row), not a character count. A fixed window
    was tried and was wrong within one commit: 8500 chars looked like "margin" against
    a table measured at ~7.8K, but header-to-end-of-table is ~9.0K, so it silently cut
    the final row. CLAUDE.md says this outright — bound on structure, since widening N
    only defers the next break. The table ends where lines stop starting with '|'."""
    start = _SKILL_TEXT.find("Gate Catalog — canonical phrasing (use these exact strings)")
    assert start != -1, "SKILL.md must carry the 'Gate Catalog — canonical phrasing' block header"
    _rest = _SKILL_TEXT[start:].splitlines()
    ...
    for _i, _line in enumerate(_rest):
        if _line.lstrip().startswith("|"):
            ...
            _end_line = _i
            break
    assert _table_started, "the Gate Catalog block has no markdown table under its header"
    block = "\n".join(_rest[:_end_line])
...
def test_no_stale_empty_snapshot_claim() -> None:
...
def test_extraction_only_fork_wired() -> None:
    """The no-cap-base fork routes a standalone instrument to the extraction-only renderer;
    the fork, the renderer invocation, and the script itself must all be present."""
    body = SKILL_MD.read_text()
    assert "No-cap-base fork" in body, "the no-cap-base fork gate must be documented in SKILL.md"
    assert "compose_extraction_report.py" in body, "SKILL.md must invoke the extraction-only renderer"
    assert (SCRIPTS_DIR / "compose_extraction_report.py").exists(), (
        "compose_extraction_report.py must exist for the extraction-only path"
    )


def test_lane1_convertible_note_interest_rate_type_enum_matches_schema() -> None:
    """The convertible_note skeleton in lane-1-pdf-docx.md must present the EXACT
    interest_rate_type enum from instruments.schema.json — guards the doc from drifting
    (the invalid 'fixed_simple' guess is exactly what this prevents)."""
    schema = json.loads(INSTRUMENTS_SCHEMA.read_text(encoding="utf-8"))
    enum = set(schema["properties"]["convertible_notes"]["items"]["properties"]["interest_rate_type"]["enum"])
    doc = (LANES_DIR / "lane-1-pdf-docx.md").read_text(encoding="utf-8")
    for v in enum:
        assert v in doc, f"lane-1 convertible_note skeleton is missing interest_rate_type value {v!r}"
    # the skeleton must actually assign a valid enum value (not just list them in prose)
    assert '"interest_rate_type": "fixed_numeric_simple"' in doc, (
        "lane-1 convertible_note skeleton must assign a real interest_rate_type enum value"
    )


def test_lane1_repipe_guidance_pins_replace_and_dup_warning() -> None:
    """The re-piping guidance must name --replace AND the dup warning, and the warning
    code must actually exist in the script (doc/code drift guard)."""
    doc = (LANES_DIR / "lane-1-pdf-docx.md").read_text(encoding="utf-8")
    assert "--replace" in doc, "lane-1 re-piping guidance must mention --replace"
    assert "W_POSSIBLE_DUPLICATE_INSTRUMENT" in doc, "lane-1 doc must document the duplicate warning"
    src = (SCRIPTS_DIR / "extract_instrument.py").read_text(encoding="utf-8")
    assert "W_POSSIBLE_DUPLICATE_INSTRUMENT" in src, (
        "extract_instrument.py must emit the W_POSSIBLE_DUPLICATE_INSTRUMENT the doc references"
    )
...
def test_skill_md_coaching_exit7_repair_dispatch() -> None:
    ...
    step11 = skill_md[start:end]
    assert "Exit 7" in step11
    assert "repair-dispatch" in step11.lower()
    idx = step11.index("Exit 7")
    window = step11[idx : idx + 300].lower()
...
def test_agent_coaching_writes_raw_markdown_no_json_escaping() -> None:
    """R2 coaching-transport fix: agents/cap-table.md's Context B section
    ...
    # Bound on the next top-level section rather than a character count — the
    # Context B block grows, and a fixed window drops its own tail without saying so.
    nxt = agent_body.find("\n## ", idx)
