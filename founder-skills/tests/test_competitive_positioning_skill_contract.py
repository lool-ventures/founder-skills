...
def test_context_b_commentary_payload_keys() -> None:
    """R2 coaching-transport fix: the Context B return is now just the
    receipt (status + output_path) — the commentary text is written to
    OUTPUT_PATH as RAW MARKDOWN, not a commentary_markdown JSON key (that
    envelope is built deterministically by the main-thread
    md_to_commentary.py adapter, not the sub-agent). The headline outcome
    fields SKILL.md's Main-Thread Return section presents are sourced from
    coaching_payload + producer artifacts + the insert_coaching.py receipt,
    not the sub-agent.
    """
    required_keys = {
        "status",
    ...
    for key in required_keys:
        assert f'"{key}"' in section, f"{AGENT_MD.name} Context B commentary payload is missing key '{key}'"
