...
RULES = REPO / "skills" / "cap-table" / "references" / "cap-table-rules.json"
...
def test_qsbs_answers_with_structured_constant():
...
def test_section_102_escalates_and_never_echoes_grant_date():
...
def test_extract_constant_recognizes_date_window_start():
...
def test_extract_constant_rejects_event_field_only_window():
...
def test_extract_constant_rejects_missing_window():
...
def test_resolve_citations_maps_source_ids():
    pack = {
        "source_bibliography": [
