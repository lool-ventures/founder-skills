...
@pytest.fixture(scope="module")
def bench_cases() -> dict[str, dict[str, Any]]:
...
def test_every_computation_case_has_canonical_values(bench_cases: dict[str, dict[str, Any]]) -> None:
...
def test_recorded_matches_known_target(case_id: str, bench_cases: dict[str, dict[str, Any]]) -> None:
...
@pytest.mark.parametrize("case_id", sorted(KNOWN_TARGETS))
def test_fresh_derivation_matches_recorded(
    case_id: str, derived: dict[str, dict[str, Any]], bench_cases: dict[str, dict[str, Any]]
) -> None:
...
def test_mfn_circular_fails_loud(derived: dict[str, dict[str, Any]]) -> None:
