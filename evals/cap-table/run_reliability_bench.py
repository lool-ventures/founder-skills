...
def run_harness_repeat(
    ...
    """Delegate reliability measurement to `cowork-harness run --repeat` and consume its rollup.

    A different measurement from the LLM-judge answer-quality path: this runs a cowork-harness
    SCENARIO (its `assert:` block) N times under the real Cowork runtime and reads the harness's
    own variance rollup — pass rate, per-assertion pass/fail attribution, verdict-signal histogram,
    cost/token totals, non-deterministic-run count — instead of the bench counting passes itself.
    The harness owns the flakiness contract (`--min-pass-rate`, `--stop-on-diverge`,
    `--max-budget-usd`); the bench just surfaces the result. Kept run dirs still feed
    bypass_telemetry.py for the pipeline-ran-vs-bypassed rate. Needs the live lane (Docker + staged
    agent + token).
    """
    cmd = [
        "cowork-harness",
    ...
    if stop_on_diverge:
        cmd.append("--stop-on-diverge")
    if max_budget_usd is not None:
        cmd += ["--max-budget-usd", str(max_budget_usd)]
    # cowork-harness 0.27.0 made an intentionally-incomplete batch a hard verdict fail by default: a
    # `--repeat` run cut short by `--stop-on-diverge` or `--max-budget-usd` now fails unless the caller
    # opts in. Both are deliberate early-stops here (we WANT to stop on the first divergence / at the
    # budget), so opt in — otherwise the rollup we came to read is reported as a failed batch.
    if stop_on_diverge or max_budget_usd is not None:
        cmd.append("--allow-budget-stop")
    print(f"[harness-repeat] {' '.join(cmd)}", file=sys.stderr, flush=True)
    ...
    for line in proc.stdout.splitlines():
        ...
        try:
            ...
            continue
        if isinstance(obj, dict) and ("rollups" in obj or "ok" in obj):
            env = obj
    if not env:
        print(
            f"no JSON envelope from cowork-harness (exit {proc.returncode})\n{proc.stderr[-1500:]}",
            file=sys.stderr,
        )
        return 1

    rollups = env.get("rollups") or []
    print("\n=== HARNESS REPEAT ROLLUP ===")
    print(f"scenario: {scenario}  |  repeats: {n}  |  min-pass-rate: {min_pass_rate}")
    for i, roll in enumerate(rollups):
        # Field names are read defensively — the envelope shape is additive across harness minors.
        rate = roll.get("passRate", roll.get("pass_rate"))
        runs = roll.get("runs", roll.get("total"))
        nd = roll.get("nonDeterministicRuns", roll.get("non_deterministic_runs"))
        print(f"rollup[{i}]: passRate={rate}  runs={runs}  nonDeterministicRuns={nd}")
        attribution = roll.get("assertions") or roll.get("perAssertion") or {}
        if attribution:
            print("  per-assertion attribution:")
            print("    " + json.dumps(attribution)[:800])
        signals = roll.get("signalHistogram") or roll.get("signals") or {}
        if signals:
            print(f"  verdict-signal histogram: {json.dumps(signals)[:400]}")
        cost = roll.get("cost") or {}
        if cost:
            print(f"  cost: {json.dumps(cost)[:200]}")
    if not rollups:
        print("(no rollups[] in envelope — check the harness version supports --repeat)")

    if out:
        Path(out).write_text(json.dumps(env, indent=2))
        print(f"\nwrote {out}")
    ok = bool(env.get("ok"))
    print(f"\nbatch verdict: {'PASS' if ok else 'FAIL'} (envelope ok={ok})")
...
def main() -> int:
    ...
    # Harness-native reliability path (0.24.0): delegate repetition to `cowork-harness run --repeat`
    # over a SCENARIO (assert-based), instead of the LLM-judge answer-quality loop above. When set,
    # this replaces the case loop entirely for the run.
    ap.add_argument(
        "--use-harness-repeat",
        metavar="SCENARIO.yaml",
        default=None,
        help="delegate to `cowork-harness run <scenario> --repeat` and consume its rollup "
        "(uses --repeats for N; live lane — needs Docker + staged agent + token)",
    )
    ap.add_argument("--min-pass-rate", type=float, default=1.0, help="harness-repeat batch threshold (0..1)")
    ap.add_argument(
        "--stop-on-diverge", action="store_true", help="harness-repeat: stop once a pass AND a fail are seen"
    )
    ap.add_argument("--max-budget-usd", type=float, default=None, help="harness-repeat: cap cumulative run cost")
    args = ap.parse_args()

    if args.use_harness_repeat:
        return run_harness_repeat(
            args.use_harness_repeat,
            max(args.repeats, 2),  # --repeat requires N>=2
            args.min_pass_rate,
            args.stop_on_diverge,
            args.max_budget_usd,
            args.out,
        )
